...
public class MemoryStore {
    ...
    private boolean contradicts(String a,String b){
        ...
        String nx=x.replaceAll("\\bне\\s+","").replaceAll("[.,!?;:]$","").trim(),ny=y.replaceAll("\\bне\\s+","").replaceAll("[.,!?;:]$","").trim();
    ...
    public void learn(String id,boolean accepted){JSONArray a=read();for(int i=0;i<a.length();i++)try{JSONObject o=a.getJSONObject(i);if(id.equals(o.optString("id"))){history(o);double c=o.optDouble("confidence",.5);c=accepted?Math.min(1,c+.10):Math.max(0,c-.15);o.put("confidence",c);o.put("status",accepted?"ACTIVE":"REJECTED");o.put("learned",true);o.put("learningSignal",accepted?"USER_CONFIRMED":"USER_REJECTED");o.put("updated",System.currentTimeMillis());o.put("version",Integer.toString(o.optInt("version",1)+1));break;}}catch(Exception ignored){}write(a);}
