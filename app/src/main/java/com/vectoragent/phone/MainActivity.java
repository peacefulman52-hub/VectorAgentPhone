...
public class MainActivity extends Activity {
    ...
    void build(){
        ...
        LinearLayout modes=new LinearLayout(this);modes.setGravity(Gravity.CENTER);Button chatMode=btn("💬 Агент"),memMode=btn("🧠 Память"),srcMode=btn("🌐 Источники"),genMode=btn("✨ Генератор"),setMode=btn("⚙");
    ...
    void showSettings(){
        ...
        addAction("Snapshot","Сохраняет память и гипотезы перед экспериментом.","Сделать Snapshot",v->{store.snapshot();toast("Snapshot сохранён");});addAction("Rollback","Возвращает последний Snapshot.","Выполнить Rollback",v->toast(store.rollback()?"Rollback выполнен":"Snapshot отсутствует"));
